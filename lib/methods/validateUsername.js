...
exports.default = (function (username) { return __awaiter(void 0, void 0, void 0, function () {
    ...
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4, auth_1.getAuth()];
            case 1:
                userId = (_a.sent()).userId;
                return [2, auth_1.getToken()
                        .then(fetch_1.default(utils_1.URLS.VALIDATEUSERNAME(userId), {
                        method: 'POST',
                        body: JSON.stringify({ nameOnPlatform: username })
                    }))
                        .then(function (res) {
                        return res.validationReports.length > 0
                            ? {
                                valid: false,
                                validationReports: res.validationReports.map(function (report) {
                                    var _a = __read(report.Message.match(/(?<=\[).*(?=\])/) || [null], 1), match = _a[0];
                                    var reportFormatted = match
                                        ? match
                                            .split(', ')
                                            .map(function (x) { return x.split(':').map(function (y) { return y.trim().split(','); }).flat(); })
